...
public:
	...
	Node(int id, String* ip, int port) {
		this->id_ = id;
	...
	virtual void acceptConnections(int listener) {
		int expectedConnections = this->maxNodes_ - (this->id_ + 1);
	...
	virtual void broadcastAddressBook() {
		...
		Message m = Message(MsgKind::Directory);
		...
		for (size_t i = 1; i < this->addressBook_->len(); ++i) {
